...
#define NFLOORS 12      /* the number of floors in the building */
...
//
semaphore mutex,mutex2,InLift[NFLOORS];
Floor_info floor[NFLOORS];
Lift_info *GetInWhichLift;  //Temporary Lift
//
...
void getintolift(Lift_info *l,int direction) {
  ...
  wait(InLift[l->position]); //critical section
  while(*waiting) {    /* for all people waiting */
	 ...
	 if(l->peopleinlift<MAXNOINLIFT && *waiting) {
		...
		Sleep(GETINSPEED);  /* wait a short time */
         wait(mutex2);  
         GetInWhichLift=l;  //---   /* tell the person which lift it is */
...
//---     /* wait for the lift to arrive */
		wait(floor[from].down_arrow);
	 	...
	 	//wait(mutex);
		l=GetInWhichLift;/* which lift are we geting in to */
